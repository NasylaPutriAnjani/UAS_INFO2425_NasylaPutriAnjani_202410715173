...
<header class="topbar">
  ...
  <nav class="nav-center" id="nav-center">
    ...
    <div id="nav-links-buyer"<?= $navGroup !== 'buyer' ? ' style="display:none"' : '' ?>>
      <button class="nav-link<?= $activePage === 'home' ? ' active' : '' ?>" onclick="showPage('home');setActive(this)">Beranda</button>
      <button class="nav-link<?= $activePage === 'catalog' ? ' active' : '' ?>" onclick="showPage('catalog');setActive(this)">Katalog</button>
      <button class="nav-link<?= $activePage === 'checkout' ? ' active' : '' ?>" onclick="showPage('checkout');setActive(this)">Checkout</button>
      <button class="nav-link<?= $activePage === 'tracking' ? ' active' : '' ?>" onclick="showPage('tracking');setActive(this)">Lacak Pesanan</button>
    </div>
    <!-- Seller nav links (shown when seller logged in) -->
    <div id="nav-links-seller"<?= $navGroup !== 'seller' ? ' style="display:none"' : '' ?>>
    ...
    <div id="nav-links-admin"<?= $navGroup !== 'admin' ? ' style="display:none"' : '' ?>>
      ...
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div class="nav-user-wrap">
          <div class="nav-user-chip" id="nav-user-chip" onclick="toggleUserDropdown()" title="<?= $user ? e($user['name']) : 'Akun' ?>">
            <div class="nav-user-avatar nav-user-avatar--role" id="nav-avatar-icon" data-role="<?= $user ? e($user['role']) : 'buyer' ?>"><?= $user ? role_icon($user['role']) : '🛒' ?></div>
